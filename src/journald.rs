...
pub mod parser {
    ...
    impl JournalExportParser {
        ...
        pub fn parse(&mut self) -> ParseResult<()> {
            if self.parse_state == ParserState::Eof {
                return ParseResult::Eof;
            }

            loop {
                use JournalExportReadError::*;
                // inv: cursor points at next character to be read
                if self.cursor - self.entry_start > self.limits.max_entry_size {
                    return self.error_state(EntryTooLarge);
                }
                if self.cursor == self.buf.upper() {
                    // If we already asked the client for more bytes, we are at
                    ...
                    if self.buffer_state == BufferState::Filled {
                        ...
                        return self.error_state(UnexpectedEof);
                    }
                ...
                self.parse_state = match self.parse_state {
                    EntryStart => {
                        if c.is_ascii_alphabetic() || c == b'_' {
                            ...
                            return self.error_state(UnexpectedCharacter(c));
                        }
                    }
                    FieldStart => match c {
                        b'\n' => {
                    ...
                    Fieldname => {
                        ...
                        if self.namelen > self.limits.max_field_name_len {
                            return self.error_state(FieldNameTooLong);
                        }
                        self.cursor += 1;
                        match c {
                            c_ if c_.is_ascii_alphanumeric() || c_ == b'_' => Fieldname,
                            b'=' => StringField,
                            b'\n' => BinaryValueLen,
                            _ => {
                                self.cursor -= 1;
                    ...
                    BinaryValueLen => {
                        ...
                        if self.cursor < len_stop || self.cursor == self.buf.upper() {
                            ...
                            if self.remaining > self.limits.max_field_value_size as u64 {
                                return self.error_state(FieldValueTooLong);
                            }
                            BinaryValue
                        }
                    }
        ...
        #[inline]
        fn error_state<T>(&mut self, r: JournalExportReadError) -> ParseResult<T> {
            self.parse_state = ParserState::Eof;
            ParseResult::Err(r)
        }
    }
